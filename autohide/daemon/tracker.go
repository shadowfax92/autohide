...
func (t *Tracker) Update(cfg *config.Config, frontmost string, visible []string) []string {
	return t.UpdateAt(cfg, frontmost, visible, time.Now())
}

// UpdateAt refreshes app activity and returns visible apps whose inactivity timer expired.
func (t *Tracker) UpdateAt(cfg *config.Config, frontmost string, visible []string, now time.Time) []string {
	t.mu.Lock()
	defer t.mu.Unlock()
